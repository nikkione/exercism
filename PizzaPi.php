...
class PizzaPi
{
    public function calculateDoughRequirement($numOfPizzaz, $peopleQuantity)
    {
        return $numOfPizzaz * (($peopleQuantity * 20) + 200);
    }

    public function calculateSauceRequirement($numOfPizzaz, $canVolume)
    {
        return $numOfPizzaz * 125 / $canVolume;
    }

    public function calculateCheeseCubeCoverage($cheeseSideLenght, $cheeseLayerThickness, $pizzaDiameter)
    {
        return floor(($cheeseSideLenght**3) / ($cheeseLayerThickness * M_PI * $pizzaDiameter));
    }

    public function calculateLeftOverSlices($pizzaNumber, $friendsNumber)
    {
        return   ($pizzaNumber*8) % $friendsNumber;
    }
}
/*Integers
Integers are whole numbers that belong to the set {..., -2, -1, 0, 1, 2, ...}. The maximum and minimum values of an integer is determined by the system PHP runs in. Typically for most modern systems, the integer is a 64-bit value -- meaning it can represent -9_223_372_036_854_775_808 to 9_223_372_036_854_775_807 inclusive.
...
Taken from PHP's documentation

Floating Point Numbers
Floating point numbers are used in PHP to represent a subset of real numbers. They start with one or more digits separated by a decimal separator.
...
Common pitfalls
Not all numbers and arithmetic operations can be performed with floating point numbers. Using floating point numbers to represent dollars and cents can lead to rounding errors.
...
Cheese comes in perfect cubes and is sold by size. Lilly calculated an equation to determine how many pizzas can of some diameter (d) made from a cheese cube of side-length (l):
